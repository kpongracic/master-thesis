...
print("base 32 kmers")
print(f'DROP_PROBABILITY: {DROP_PROBABILITY}')
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if seq_length < LENGTH:
            end = start + (seq_length // KMER_SIZE) * KMER_SIZE
        
        seq = self.sequences[y][start:end]
        
        x = encode_letters(seq)
        x = torch.tensor(x)
        x = F.one_hot(x, 4)
        x = x.float()
        x = x.view(-1, 128)
        ...
        return x, torch.tensor(y), mask_index
...
def pad_collate_and_mask_fn(batch):
    X, y, mask_index = zip(*batch)

    src_padding_mask = [torch.tensor([False] * (seq.shape[0] + 1)) for seq in X] # +1 zbog cls tokena
    src_padding_mask = pad_sequence(src_padding_mask, padding_value=True, batch_first=True)
    
    for i in range(len(mask_index)):
        for j in range(len(mask_index[i])):
            src_padding_mask[i][mask_index[i][j]] = True
...
train_loader = DataLoader(train, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, collate_fn=pad_collate_and_mask_fn)
...
checkpoint_callback = ModelCheckpoint(
        ...
        monitor='valid_acc',
        filename='drop0.1-{epoch:02d}-{valid_acc:.3f}',
        save_top_k=3,
        mode='max',
    )
