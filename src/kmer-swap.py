...
name_dict_file = '../kmer_variations/chr13_kmersize9_start0_end10000000_kmer_dict.pkl'


print("swap 32-mers")
print(f'SWAP_PROBABILITY: {SWAP_PROBABILITY}')
...
################################################
################################################
kmers_dict = pickle.load(open(name_dict_file,"rb"))

print(f'dict len {len(kmers_dict)}')

def swap_kmer_function(seq):
    seq_length = len(seq)
    changes = list()
    for i in range(0, seq_length - KMER_SWAP_SIZE + 1, KMER_SWAP_SIZE):

        if random.random() > SWAP_PROBABILITY:
            continue

        kmer_key = seq[i:i + KMER_SWAP_SIZE]

        if kmer_key not in kmers_dict:
            continue

        swap_index = random.randint(0, len(kmers_dict[kmer_key]) - 1)
        changes.append((i, kmers_dict[kmer_key][swap_index]))
    ...
    new_seq = ""
    for start_index, change_seq in changes:
        new_seq += seq[current_base_index:start_index]
        new_seq += change_seq
        current_base_index = start_index + KMER_SWAP_SIZE

    new_seq += seq[current_base_index:]
    return new_seq
################################################
################################################
################################################
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seq = swap_kmer_function(self.sequences[y][start:end])
        seq_length = min(len(seq), LENGTH)
        
        seq_end = seq_length
        if seq_length < LENGTH:
            seq_end = (seq_length // KMER_SIZE) * KMER_SIZE
        
        x = encode_letters(seq[:seq_end])
        x = torch.tensor(x)
        x = F.one_hot(x, 4)
        x = x.float()
        x = x.view(-1, 128)
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if seq_length < LENGTH:
            end = (seq_length // KMER_SIZE) * KMER_SIZE
        
        x = encode_letters(seq[start:end])
        x = torch.tensor(x)
        x = F.one_hot(x, 4)
        x = x.float()
        x = x.view(-1, 128)
...
validation_loader = DataLoader(validation, batch_size=BATCH_SIZE, num_workers=1, collate_fn=pad_collate_fn)
test_loader = DataLoader(test, batch_size=BATCH_SIZE, num_workers=1, collate_fn=pad_collate_fn)
...
class MyModel(LightningModule):
    def __init__(self):
        ...
        self.input_linear_layer = nn.Linear(128, 256)
...
checkpoint_callback = ModelCheckpoint(
        dirpath=f"../model-checkpoints/kmer/{NUMBER_OF_SEQ_TRAIN}",
        monitor='valid_acc',
        filename='swap0.1-{epoch:02d}-{valid_acc:.3f}',
        save_top_k=3,
        mode='max',
    )
